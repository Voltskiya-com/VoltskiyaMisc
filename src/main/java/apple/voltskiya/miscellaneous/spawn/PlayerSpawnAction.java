...
public class PlayerSpawnAction {
    ...
    RepeatingActionManager actionManager = new RepeatingActionManager(VoltskiyaPlugin.get())
            .registerInit(this::runInit)
            .registerFinally(this::runFinally)
            .registerAction(new RepeatableActionImpl(SPAWN,
                                                     this::showTitle,
                                                     duration,
                                                     duration / VOLTSKIYA.length()
            ))
            .registerAction(new RepeatableActionImpl(SIMMER,
                                                     this::simmerTitle,
                                                     PlayerSpawnConfig.get().getSimmer(),
                                                     5
            ));
    ...
    public PlayerSpawnAction(Location spawnLocation, HumanEntity player) {
        ...
        this.player = player;
        this.welcome = Component.text("Welcome to ").style(Style.style(TextColor.color(0xacacad)));
        actionManager.startActionAndStart(SPAWN);
    }

    private ActionReturn simmerTitle(ActionMeta meta) {
//            player.getWorld().playSound(Sound.sound(org.bukkit.Sound.BLOCK_SAND_HIT.key(), Sound.Source.PLAYER, 1f, 1f));
    ...
    protected void runFinally() {
        @NotNull TextComponent volt = Component
                .text(VOLTSKIYA)
        ...
        Title.Times timings = Title.Times.times(Duration.ZERO, Duration.ofSeconds(3L), Duration.ofSeconds(2L));
        player.resetTitle();
        player.showTitle(Title.title(message, Component.empty(), timings));
        player.setInvulnerable(false);
        player
                .getWorld()
                .playSound(Sound.sound(org.bukkit.Sound.UI_TOAST_CHALLENGE_COMPLETE.key(),
                                       Sound.Source.MASTER,
                                       1f,
                                       1f
                ));
    ...
    private ActionReturn showTitle(ActionMeta meta) {
        ...
        if (VOLTSKIYA.length() <= currentTick + 1) {
            volt = Component.text(VOLTSKIYA).style(Style.style(TextColor.color(0x545454), TextDecoration.ITALIC));
            skiya = Component.empty();
        } else {
            volt = Component
                    .text(VOLTSKIYA.substring(0, currentTick + 1))
                    .style(Style.style(TextColor.color(0x545454), TextDecoration.ITALIC));
            skiya = Component
                    .text(VOLTSKIYA.substring(currentTick + 1))
                    .style(Style.style(TextColor.color(0x545454), TextDecoration.OBFUSCATED));
        }
        @NotNull Component message = Component.join(JoinConfiguration.noSeparators(), welcome, volt, skiya);
        Title.Times timings = Title.Times.times(Duration.ZERO, Duration.ofSeconds(5L), Duration.ofSeconds(3L));
        player.resetTitle();
        player.showTitle(Title.title(message, Component.empty(), timings));
        if (meta.isLastRun()) {
            ...
            player
                    .getWorld()
                    .playSound(Sound.sound(org.bukkit.Sound.BLOCK_BEACON_DEACTIVATE.key(),
                                           Sound.Source.PLAYER,
                                           1f,
                                           1f
                    ));
