...
public class SpawnJoinGuiMain extends InventoryGuiPageImplACD<SpawnJoinGui> {
    ...
    public void initialize() {
        for (Map.Entry<Integer, PlayerSpawnpoints> spawnpoint : PlayerSpawnDatabase.get().getSpawnPoints().entrySet()) {
            setSlot(slotImpl(e -> spawnpoint.getValue().spawn(e.getWhoClicked()), spawnpoint.getValue().toItem()),
                    spawnpoint.getKey()
            );
        }
    }
